...
function adjustPrice($price, $product) {
    ...
    if ( $product->get_data()['id'] != $product_id || $date > date('Y-m-d H:i:s') || ( $units_sold ) > intval( $range_price_3 ) ) return $price;

    if ( $units_sold <= intval( $def_val_1 ) ) $price = $range_price_1;
    elseif( $units_sold > intval( $def_val_1 ) && $units_sold <= intval( $def_val_2 ) ) $price = $range_price_2;
    elseif( $units_sold > intval( $def_val_2 ) && $units_sold <= intval( $def_val_3 ) ) $price = $range_price_3;
